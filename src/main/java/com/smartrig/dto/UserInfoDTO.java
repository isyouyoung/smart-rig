...
public record UserInfoDTO(
        ...
        @Size(min = 4, max = 16, message = "아이디는 최소 4글자에서 16글자까지 입력 가능합니다.")
        String userId,

        @NotBlank(message = "이름은 필수 입력 사항입니다.")
        @Size(max = 10, message = "이름은 10글자까지 입력 가능합니다.")
        String userName,

        @NotBlank(message = "비밀번호는 필수 입력 사항입니다.")
        @Size(max = 16, message = "비밀번호는 16글자까지 입력 가능합니다.")
        String password,

        @NotBlank(message = "이메일은 필수 입력 사항입니다.")
        @Size(max = 30, message = "이메일은 30글자까지 입력 가능합니다.")
        @Email String email,

        @NotBlank(message = "주소는 필수 입력 사항입니다.")
        @Size(max = 30, message = "주소는 30글자까지 입력 가능합니다.")
        String addr1,

        @NotBlank(message = "상세 주소는 필수 입력 사항입니다.")
        @Size(max = 100, message = "상세 주소는 100글자까지 입력 가능합니다.")
        String addr2,
        String regId,
        String regDt,
        String chgId,
